...
`MerkleInclusion`, `CredentialVerification`, `ActionRangeProof`, `ActionTimeWindow`, `ActionSetMembership`, `StatsHonestComputation`, and others. All circuits live in [zkp/circuits/](../zkp/circuits/). See [zk-action-logs.md](zk-action-logs.md) for the action-log selective-disclosure design.
...
### Paillier Homomorphic Encryption (custom Rust implementation)

Additive homomorphic encryption supports privacy-preserving aggregation where customers submit encrypted contributions and the server computes the sum without ever decrypting individual values.
...
**Caveat:** the implementation uses `num-bigint`, which is not constant-time. The module is explicitly labeled `NEEDS_CRYPTO_REVIEW` and must not be deployed for production secret aggregation without cryptographer review. See [homomorphic-encryption.md](homomorphic-encryption.md).
...
| Big integers | `num-bigint` 0.4 | Paillier HE arithmetic (not constant-time) |
