...
4. **On the VM**, create the server env (secrets stay on the VM):
   ...
   ```
5. **(Optional) Solana** — see §5 / `docs/demo-anchoring.md`. Default is off
...
Full detail + the verified Bitcoin proof: **`docs/demo-anchoring.md`**.
...
- **Solana: optional.** Free on devnet but the faucet is currently gated; set
  ...
  `docs/demo-anchoring.md` if you want the on-chain memo too.
