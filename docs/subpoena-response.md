...
Cross-references: `docs/threat-model.md`, `docs/key-rotation.md`, `docs/disaster-recovery.md` §9 (GDPR-wipe is the inverse procedure).
...
| ZK trapdoor / toxic waste | Should not exist if ceremony was honest (`zkp/ceremony/README.md`). If it does, see `docs/disaster-recovery.md` §8. Not a server-side artefact. |
...
| Cross-tenant statistical inferences from aggregated cohort numbers | `core/src/dp/*` applies k-anonymity (`k_anonymity::suppress`) and Laplace / Gaussian DP noise. Per-period ε budget caps de-anonymisation. See "Privacy bound" section below. |
...
- The OPRF seed has not leaked (§4 of `docs/key-rotation.md`).
...
- `core/src/dp/k_anonymity.rs` suppresses cohorts smaller than `k_threshold` (default 10). Returns empty.
- `core/src/dp/laplace.rs` / `core/src/dp/gaussian.rs` add calibrated noise to numerical aggregates.
- `core/src/dp/budget.rs` tracks per-period ε budget. After the budget is exhausted, further queries return cached responses (no fresh information leaks).
...
| Anchor timing / batching cadence | Public. The batching interval (`SAURON_ACTION_ANCHOR_INTERVAL_SECS`) is operator config and is documented in `docs/operations.md`. |
