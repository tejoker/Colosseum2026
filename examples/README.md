...
| `typescript-quickstart/` | userAuth, registerLlmAgent, signed call, over-limit payment denial | Node 18+, `npm install`, built `agentic/`, agent-action-tool |
...
| `vercel-ai/` | `sauronTools()` wrapping an `ai` tool set (illustrative) | Node 18+, `npm install`, built `agentic/` |
